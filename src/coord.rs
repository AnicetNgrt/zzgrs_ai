...
impl Coord {
    pub fn add_nowrap(&self, other: &Coord, w: i8, h: i8) -> Option<Coord> {
        let res = Coord { x: self.x + other.x, y: self.y + other.y };
    ...
    pub fn add_wrap(&self, other: &Coord, w: i8, h: i8) -> Coord {
        Coord { x: (self.x + other.x).rem_euclid(w), y: (self.y + other.y).rem_euclid(h) }
    }
    ...
    pub fn distance_wrap(&self, other: &Coord, w: i8, h: i8) -> i8 {
        ...
        dist_nowrap.min(dist_wrapx).min(dist_wrapy).min(dist_wrapboth)
    }

    pub fn distance_nowrap(&self, other: &Coord, w: i8, h: i8) -> i8 {
        let a = Coord { x: self.x.rem_euclid(w), y: self.y.rem_euclid(h) };
        let b = Coord { x: other.x.rem_euclid(w), y: other.y.rem_euclid(h) };
        Self::distance(&a, &b)
    }
